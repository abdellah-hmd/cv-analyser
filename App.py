...
connection = pymysql.connect(host='localhost', user='root', password='Ah@456123', db='cvanalyzerdb')
...
def run():
    st.title("Smart CV Analyser")
    st.sidebar.markdown("# Choose User")
    activities = ["Normal User", "Admin"]
    choice = st.sidebar.selectbox("Choose among the given options:", activities)
    # link = '[©Developed by Spidy20](http://github.com/spidy20)'
    # st.sidebar.markdown(link, unsafe_allow_html=True)
    img = Image.open('./Logo/cv-logo.jpg')
    img = img.resize((250, 250))
    st.image(img)

    # Create the DB
    db_sql = """CREATE DATABASE IF NOT EXISTS cvanalyzerdb;"""
    cursor.execute(db_sql)
    connection.select_db("cvanalyzerdb")
